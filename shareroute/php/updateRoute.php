<?php
if(!empty($_POST["uid"])){
   $uid = $_POST["uid"];
   $routeName = $_POST["routeName"];
   $pinname = $_POST["pinnameList"];
   $lat = $_POST["latList"];
   $lng = $_POST["lngList"];
   $name = $_POST["name"];
   $id = $_POST["id"];
   $nameList = explode(",", $pinname);
   $latList = explode(",", $lat);
   $lngList = explode(",", $lng);
   $count = 0;
 try {
    $dbh = new PDO('mysql:host=localhost;dbname=ordernavi;charset=utf8', "root", "");
    $sql = "DELETE FROM route WHERE uid = '$uid' AND routename = '$routeName'";
    $res = $dbh->query($sql);
    for($i = 0; $i < count($latList); $i++ ){
    $sql = "INSERT INTO route (uid,routename,pinname,lat,lng,number,name,id) VALUES ('$uid','$routeName','$nameList[$i]','$latList[$i]','$lngList[$i]','$i','$name','$id')";
    $res = $dbh->query($sql);
    }
    echo json_encode("OK");
 }catch (PDOException $e) {
    echo json_encode("$e");
    die();
 }
 $dbh = null;
} else {
    echo json_encode("NG");
}

...
import { getmyRoutename, setRoute } from "./ajax.js";
...
const save = async () => {
  ...
  if (routeName !== "" && latList.length !== 0 && state !== false) {
    ...
    const id = createId();
    setRoute(routeName, nameList, latList, lngList, id);
    alert("保存しました");
    deleteMarkers();
...
const checkname = async () => {
  const nameElement = await getmyRoutename();
  const nameList = [];
  if (nameElement !== "") {
